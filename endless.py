...
SCREEN_WIDTH = 800
SCREEN_HEIGHT = int(SCREEN_WIDTH * 0.80)
...
class Soldier(pygame.sprite.Sprite):
    ...
    def move(self, moving_left, moving_right):
        ...
        #assign movement variables if moving left or moving right
        if moving_left:
            dx = -self.speed
            self.flip = True
            self.direction = -1
...
enemy = Soldier('enemy', 400, 246,3, 5)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_ESCAPE:
                run = False
        #if keyboard button is released
